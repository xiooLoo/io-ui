<template>
  <div class="yxt-biss-datacenter-content">
...
<script>
export default {
  name: 'YxtBissDataCenterPanelContent',
  props: {
    panelInfo: {
      type: Object,
      default: () => {
  ...
  data() {
    return {
      ...
      execFormKey: {
        field: this.getExecFormValue('field', this.panelInfo),
        lineCount: this.getExecFormValue('lineCount', this.panelInfo)
      }
    }
  },
  watch: {
    panelInfo: {
      handler(nval) {
        ...
        this.execFormKey = {
          field: this.getExecFormValue('field', nval),
          lineCount: this.getExecFormValue('lineCount', nval)
        }
      },
      deep: true
    },
  ...
  methods: {
    ...
    execActions(key, obj) {
      if (key === 'uploadSuccess') {
        ...
        // const arr = obj.item.arr
        // const res = obj.value
        // let temps = arr.filter(item => res.includes(item.value))
        // console.log('多选结果过滤：', temps)
        // this.execFormKey.activeArr = temps
      }
    },
    _submit(btn) {
      ...
      tempArr.forEach(i => {
        ...
        if (temp.comp && temp.comp.xyz && temp.comp.xyz.i === currentGrid.xyz.i) {
          ...
          case 'watchExecFormKey': {
            let field = temp.comp.panel.content.exec_rows.find(j => j.id === 'field')
            let lineCount = temp.comp.panel.content.exec_rows.find(j => j.id === 'lineCount')
            field.value = query.field
            lineCount.value = query.lineCount
            break
          }
          default:
            break

<template>
  <div class="yxt-carousel-panel-extyle">
    ...
    <yxt-collapse :confige="extyleConfig">
      <div slot="titleSlot">
        ...
        <to-row>
          <span class="tip-label">标题显示</span>
          <to-radio-group v-model="extyleFormKey.titleShow">
            <to-radio v-for="im in titleShowArr" :key="im.id" :label="im.value">{{ im.label }}</to-radio>
          </to-radio-group>
        </to-row>
        <to-row>
...
<script>
export default {
  name: 'YxtFuncCarouselPanelExtyle',
  props: {
    panelInfo: {
      type: Object,
      default: () => {
  ...
  data() {
    return {
      ...
      titleShowArr: this.panelInfo.panel.extyle.titleShow.arr,
      ...
      extyleFormKey: {
        ...
        titleShow: this.getExtyleValue('titleShow', this.panelInfo).current,
    ...
    panelInfo: {
      handler(nval) {
        this.extyleFormKey = {
          ...
          titleShow: this.getExtyleValue('titleShow', nval).current,
      ...
      tempArr.forEach(i => {
        ...
        if (temp.comp && temp.comp.xyz && temp.comp.xyz.i === currentGrid.xyz.i) {
          ...
          case 'watchExtyleFormKey': {
            ...
            temp.comp.panel.extyle.titleShow.current = query.titleShow

...
<script>
export default {
  name: 'YxtFuncCarouselPanelContent',
  props: {
    panelInfo: {
      type: Object,
      default: () => {
  ...
  data() {
    return {
      ...
      baseFormKey: {
        ...
        interval: this.getBaseFormValue('interval', this.panelInfo)
      },
      execFormKey: {
        carouoselPic: this.getExecFormValue('carouoselPic', this.panelInfo),
        carouoselUrl: this.getExecFormValue('carouoselUrl', this.panelInfo)
      }
    }
  },
  watch: {
    panelInfo: {
      handler(nval) {
        this.baseFormKey = {
          ...
          // title: nval.title,
          interval: this.getBaseFormValue('interval', nval)
        }
        this.execFormKey = {
          carouoselPic: this.getExecFormValue('carouoselPic', nval),
          carouoselUrl: this.getExecFormValue('carouoselUrl', nval)
        }
      },
      deep: true
    },
      ...
      tempArr.forEach(i => {
        ...
        if (temp.comp && temp.comp.xyz && temp.comp.xyz.i === currentGrid.xyz.i) {
          ...
          case 'watchBaseFormKey': {
            ...
            let interval = temp.comp.panel.content.base_rows.find(j => j.id === 'interval')
            title.value = query.title
            interval.value = query.interval
            break
          }
          case 'watchExecFormKey': {
            let carouoselPic = temp.comp.panel.content.exec_rows.find(j => j.id === 'carouoselPic')
            let carouoselUrl = temp.comp.panel.content.exec_rows.find(j => j.id === 'carouoselUrl')
            carouoselPic.value = query.carouoselPic || '上传图片-待补充...'
            carouoselUrl.value = query.carouoselUrl
            break
          }
          default:
            break

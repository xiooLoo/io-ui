  ...
  export default {
    ...
    render(h) {
      ...
      const scrollBtn = scrollable
        ? [
          <span class={['to-tabs__nav-prev', scrollable.prev ? '' : 'is-disabled']} on-click={scrollPrev}><i class="to-icon-arrow-left"></i></span>,
          <span class={['to-tabs__nav-next', scrollable.next ? '' : 'is-disabled']} on-click={scrollNext}><i class="to-icon-arrow-right"></i></span>
        ] : null;
      ...
      const tabs = this._l(panes, (pane, index) => {
        ...
        const btnClose = closable
          ? <span class="to-icon-close" on-click={(ev) => { onTabRemove(pane, ev); }}></span>
          : null;
      ...
      return (
        <div class={['to-tabs__nav-wrap', scrollable ? 'is-scrollable' : '', `is-${ this.rootTabs.tabPosition }`]}>
          {scrollBtn}
          <div class={['to-tabs__nav-scroll']} ref="navScroll">
            <div
              class={['to-tabs__nav', `is-${ this.rootTabs.tabPosition }`, stretch && ['top', 'bottom'].indexOf(this.rootTabs.tabPosition) !== -1 ? 'is-stretch' : '']}
              ref="nav"
              style={navStyle}
              role="tablist"

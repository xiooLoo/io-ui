...
export default {
  ...
  methods: {
    ...
    renderEmptyText(h) {
      return (
        <div class="to-cascader-menu__empty-text">{ this.t('el.cascader.noData') }</div>
      );
    },
    renderNodeList(h) {
      const { menuId } = this;
      ...
      return [
        ...nodes,
        isHoverMenu ? <svg ref='hoverZone' class='to-cascader-menu__hover-zone'></svg> : null
      ];
    }
  },
  ...
  render(h) {
    ...
    return (
      <to-scrollbar
        ...
        role="menu"
        id={ menuId }
        class="to-cascader-menu"
        wrap-class="to-cascader-menu__wrap"
        view-class={{
          'to-cascader-menu__list': true,
          'is-empty': isEmpty
        }}
        { ...events }>

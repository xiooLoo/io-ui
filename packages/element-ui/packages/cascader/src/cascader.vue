<template>
  <div
    ...
    <to-input
      ...
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="to-input__icon to-icon-circle-close"
    ...
    <div v-if="multiple" class="to-cascader__tags">
      <to-tag
      ...
      <input
        ...
        type="text"
        class="to-cascader__search-input"
    ...
    <transition name="to-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['to-popper', 'to-cascader__dropdown', popperClass]">
        <to-cascader-panel
          ref="panel"
          v-show="!filtering"
          ...
          @close="toggleDropDownVisible(false)"></to-cascader-panel>
        <to-scrollbar
          ref="suggestionPanel"
          v-if="filterable"
          v-show="filtering"
          tag="ul"
          class="to-cascader__suggestion-panel"
          view-class="to-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
              ...
              :key="item.uid"
              :class="[
                'to-cascader__suggestion-item',
                item.checked && 'is-checked'
              ]"
              :tabindex="-1"
              @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="to-icon-check"></i>
            </li>
          </template>
          <slot v-else name="empty">
            <li class="to-cascader__empty-text">{{ t('el.cascader.noMatch') }}</li>
          </slot>
        </to-scrollbar>
      </div>
    </transition>
...
import ToTag from '../../../packages/tag';
import ToScrollbar from '../../../packages/scrollbar';
import ToCascaderPanel from '../../../packages/cascader-panel';
...
export default {
  ...
  computed: {
    realSize() {
      const _toFormItemSize = (this.toFormItem || {}).toFormItemSize;
      return this.size || _toFormItemSize || (this.$ELEMENT || {}).size;
    },
    tagSize() {
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.to-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.to-cascader-menu');
          firstNode = firstMenu.querySelector('.to-cascader-node[tabindex="-1"]');
        }
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.to-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.to-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.to-cascader__suggestion-list');

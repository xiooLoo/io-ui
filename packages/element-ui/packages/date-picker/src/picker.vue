<template>
  <to-input
    class="to-date-editor"
    :class="'to-date-editor--' + type"
  ...
  <div
    class="to-date-editor to-range-editor to-input__inner"
    :class="[
      'to-date-editor--' + type,
      pickerSize ? `to-range-editor--${ pickerSize }` : '',
    ...
    v-clickoutside="handleClose"
    v-else>
    <i :class="['to-input__icon', 'to-range__icon', triggerClass]"></i>
    <input
      autocomplete="off"
    ...
    <span class="to-range-separator">{{ rangeSeparator || '-' }}</span>
    ...
    <i
      ...
      v-if="haveTrigger"
      :class="[showClose ? '' + clearIcon : '']"
      class="to-input__icon to-range__close-icon">
    </i>
  </div>
</template>
...
export default {
    ...
    pickerVisible(val) {
      ...
      if (val) {
        ...
        if (this.validateEvent) {
          this.dispatch('ToFormItem', 'el.form.blur');
        }
        this.$emit('blur', this);
        this.blur();
      }
  ...
  computed: {
    ...
    triggerClass() {
      return this.prefixIcon || (this.type.indexOf('time') !== -1 ? 'to-icon-time' : 'to-icon-date');
    },

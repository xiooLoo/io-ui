...
export default {
  ...
  computed: {
    ...
    formatedToday() {
      return this.toCalendar.formatedToday;
    },
        ...
        <tbody>
          {
            this.rows.map((row, index) => <tr
              class={{
                'to-calendar-table__row': true,
                'to-calendar-table__row--hide-border': index === 0 && this.hideHeader
              }}
              key={index}>
              {
